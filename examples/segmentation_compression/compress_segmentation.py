...
import distiller.apputils.image_segmentation as segmentation
import distiller.apputils as apputils
import parser
import os
import numpy as np
from ptq_lapq import image_segmentation_ptq_lapq
...
def main():
    # Parse arguments
    args = parser.add_cmdline_args(segmentation.init_segmentation_compression_arg_parser(True)).parse_args()
    app = SegmentationCompressorSampleApp(args, script_dir=os.path.dirname(__file__))
...
def handle_subapps(model, criterion, optimizer, compression_scheduler, pylogger, args):
    def load_test_data(args):
        test_loader = segmentation.load_data(args, load_train=False, load_val=False, load_test=True)
        return test_loader
    ...
    if args.greedy:
        ...
        segmentation.acts_quant_stats_collection(model, criterion, pylogger, args, save_to_file=True)
        do_exit = True
    elif args.activation_histograms:
        segmentation.acts_histogram_collection(model, criterion, pylogger, args)
        do_exit = True
        ...
        if args.quantize_eval and args.qe_lapq:
            image_segmentation_ptq_lapq(model, criterion, pylogger, args)
        else:
            test_loader = load_test_data(args)
            segmentation.evaluate_model(test_loader, model, criterion, pylogger,
                segmentation.create_activation_stats_collectors(model, *args.activation_stats),
...
class SegmentationCompressorSampleApp(segmentation.SegmentationCompressor):
...
def sensitivity_analysis(model, criterion, data_loader, loggers, args, sparsities):
    ...
    if not isinstance(loggers, list):
        loggers = [loggers]
    test_fnc = partial(segmentation.test, test_loader=data_loader, criterion=criterion,
                       loggers=loggers, args=args,
                       activations_collectors=segmentation.create_activation_stats_collectors(model))
...
def greedy(model, criterion, optimizer, loggers, args):
    train_loader, val_loader, test_loader = segmentation.load_data(args)

    test_fn = partial(segmentation.test, test_loader=test_loader, criterion=criterion,
                      loggers=loggers, args=args, activations_collectors=None)
    train_fn = partial(segmentation.train, train_loader=train_loader, criterion=criterion, args=args)
