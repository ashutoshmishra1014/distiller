...
def image_segmentation_ptq_lapq(model, criterion, loggers, args):
    args = deepcopy(args)

    effective_test_size_bak = args.effective_test_size
    args.effective_test_size = args.lapq_eval_size
    eval_data_loader = segmentation.load_data(args, load_train=False, load_val=False, load_test=True, fixed_subset=True)

    args.effective_test_size = effective_test_size_bak
    test_data_loader = segmentation.load_data(args, load_train=False, load_val=False, load_test=True)
    ...
    def eval_fn(model):
        if memoized_data_loader:
            ...
            _, _, loss = segmentation.test(eval_data_loader, model, criterion, loggers, None, args)
        return loss

    def test_fn(model):
        top1, top5, loss = segmentation.test(test_data_loader, model, criterion, loggers, None, args)
...
if __name__ == "__main__":
    parser = segmentation.init_classifier_compression_arg_parser(include_ptq_lapq_args=True)
    args = parser.parse_args()
    args.epochs = float('inf')  # hack for args parsing so there's no error in epochs
    cc = segmentation.SegmentationCompressor(args, script_dir=os.path.dirname(__file__))
    image_segmentation_ptq_lapq(cc.model, cc.criterion, [cc.pylogger, cc.tflogger], cc.args)
